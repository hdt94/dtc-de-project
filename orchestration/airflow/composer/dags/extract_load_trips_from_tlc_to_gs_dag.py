...
@task.virtualenv(
    task_id="create_job_extract_load_trips_from_tlc_to_gs",
    requirements=[CLOUD_RUN_REQUIREMENT],
    system_site_packages=False,
)
def create_job_extract_load(bucket_name, image, parent, vehicle_type, year, data_interval_end=None, run_id=None):
    """
    Create Cloud Run Job to extract and load based on scheduled DagRun:
        extract_load(bucket_name, image, parent, vehicle_type)
    This call will raise error if URL is not found.

    Create Cloud Run Job to extract and load based on explicit year parameter:
...
@task.virtualenv(
    task_id="delete_job_extract_load_trips_from_tlc_to_gs",
    requirements=[CLOUD_RUN_REQUIREMENT],
...
@task.virtualenv(
    task_id="run_job_extract_load_trips_from_tlc_to_gs",
    requirements=[CLOUD_RUN_REQUIREMENT],
    system_site_packages=True,
)
...
with DAG(
    ...
    job_name = create_job_extract_load.partial(**partials).expand(**expands)
    job_name = run_job(job_name)
    delete_job(job_name)
